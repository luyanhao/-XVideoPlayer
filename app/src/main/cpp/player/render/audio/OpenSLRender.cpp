...
void OpenSLRender::Init() {
    ...
    LOGCATE("OpenSLRender::Init Success!!!!!");
}
...
int OpenSLRender::CreateOutputMixer() {
    ...
    do {
        ...
        if(result != SL_RESULT_SUCCESS) {
            LOGCATE("OpenSLRender::CreateOutputMixer Realize fail. result=%d", result);
            break;
        }
    } while (false);
...
void OpenSLRender::AudioPlayerCallback(SLAndroidSimpleBufferQueueItf bufferQueueItf, void *context) {
